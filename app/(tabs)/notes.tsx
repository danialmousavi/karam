  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.headerRow}>
        <TouchableOpacity 
          style={[styles.addFolderBtn, { backgroundColor: colors.surface, borderColor: colors.border }]}
          onPress={() => setFolderModalVisible(true)}
        >
          <Feather name="folder-plus" size={20} color={colors.primaryDark} />
        </TouchableOpacity>
        <Text style={[styles.headerTitle, { color: colors.text }]}>پوشه‌های من</Text>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            ...
            onPress={() => router.push({ pathname: '/notes/[id]', params: { id: item.id, name: item.name } })}
          >
            <TouchableOpacity 
            ...
            <Feather name="folder" size={40} color={item.color || colors.primary} style={{ marginBottom: 10 }} />
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  headerRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginHorizontal: 20, marginTop: 60, marginBottom: 20 },
  headerTitle: { fontFamily: 'Vazir-Bold', fontSize: 22 },
  addFolderBtn: { width: 44, height: 44, borderRadius: 14, borderWidth: 1, justifyContent: 'center', alignItems: 'center', elevation: 1 },
  listContainer: { paddingHorizontal: 10, paddingBottom: 120 },
  folderCard: { flex: 1, margin: 10, padding: 20, borderRadius: 16, borderWidth: 1, alignItems: 'center', justifyContent: 'center', minHeight: 120, elevation: 2, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 4, shadowOffset: { width: 0, height: 2 } },
  deleteIcon: { position: 'absolute', top: 12, left: 12, padding: 4 },
  folderName: { fontFamily: 'Vazir-Bold', fontSize: 16 },
});